...
class RecyclerNote(var context: Context,
                   private var mData: ArrayList<MemoEntity>,
                   var mClick: ItemClickListener)
    : RecyclerView.Adapter<RecyclerNote.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(LayoutInflater.from(context).inflate(R.layout.sample_list_2, parent, false))
    }

    fun addItem(value: MemoEntity) {
        mData.add(value)
...
//        notifyItemRangeChanged(position, mData.size)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.time.text = mData[position].note
    ...
    interface ItemClickListener {
        fun onItemClick(position: MemoEntity)
    }
}